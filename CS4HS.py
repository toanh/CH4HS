...
class Graphics:
    ...
    def _drawImage(self, image, x, y, width=None, height=None, rotation=0, anchorX=None, anchorY=None, opacity=None,
                   rect=None):
        try:
            self.t.shape(image)
            ...
            self.t.pu()
            self.t.setpos(
                self._convXY(x + int(self.images[image].width() / 2), y + int(self.images[image].height() / 2)))
            self.t.pd()
            self.t.stamp()
    ...
    def _drawText(self, text, x, y, fontName="Arial", fontSize=10, color=(1, 1, 1, 1), anchorX="left",
                  anchorY="bottom"):
        self.t.pu()
    ...
    def drawCircle(self, color, pos, radius, width=0):
        kwargs = {"color": color, \
                  ...
                  "radius": radius, \
                  "width": width}
        self.commands.append([self._drawCircle, kwargs])

    def _drawCircle(self, color, pos, radius, width=0):
        turtle.pu()
        turtle.setpos(pos)
        turtle.pd()
    ...
    def drawPixel(self, color, pos):
        kwargs = {"color": color, \
                  "pos": pos}
        self.commands.append([self._drawPixel, kwargs])

    def _drawPixel(self, color, pos):
